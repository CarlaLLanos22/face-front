...
function Turnos() {
    ...
    const [horarios, setTurnos] = useState([]);
    ...
    useEffect(() => {
      fetch("http://localhost:3000/horario",{
        ...
        .then((res) => res.json())
        .then((horarios) => setTurnos(horarios));
    ...
    const eliminarHorario = async (horarioId) => {
      if (window.confirm("¿Desea eliminar ?")) {
        ...
        if (res.ok) {
          setTurnos(horarios.filter((horario) => horario.horario.id_horario !== horarioId));
          limpiarForm()
        } else {
    ...
    const agregarHorario = async () => {
      ...
      if (res.ok) {
        ...
        setTurnos([...horarios, { horario: horarioNuevo, medico: { id_medico: horarioNuevo.id_medico } }]);
        limpiarForm()
      } else {
        alert(mensajeError())
      }
            ...
            <main className="md:w-2/5  xl:w-4/5 px-5 py-10 bg-gray-200">
                <div className="flex flex-row items-start">
                    ...
                    <div className="py-2 w-full ml-5 t-0 -mt-12">
                      <h2 className="text-3xl font-light text-center mt-20">Listado de Turnos</h2>
                        ...
                        <div className="align-middle w-full inline-block shadow overflow-hidden sm:rounded-lg border-b border-gray-200">
                            <table className="w-full ">
                                ...
                                {/* Inicio del Listado */}
                                <tbody id="listado-Turnos" className="bg-white">
                                    ...
                                    {horarios.map((item)=>(
                                        <tr key={item.id_horario}>
                                            ...
                                            { item.horario.hora_inicio != 0 && (<th className="px-6 py-3 border-b border-gray-200  text-center text-xs leading-4 font-medium text-gray-600 uppercase tracking-wider">
                                              ✅
                                            </th>)}
                                            { item.horario.hora_inicio == 0 && (<th className="px-6 py-3 border-b border-gray-200  text-center text-xs leading-4 font-medium text-gray-600 uppercase tracking-wider">
                                              ❌
